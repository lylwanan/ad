...
public class ADXUpStreamServiceImp implements UpStreamService {
    ...
    private ADXRequest sigmobRequestToADXRequest(SigmobRequest sigmobRequest) {
        ...
        List impList = new LinkedList<com.ad.model.adx.request.Imp>();
    ...
    private SigmobResponse ADXResponseToSigmobResponse(ADXResponse adxResponse) {
        SigmobResponse sigmobResponse = new SigmobResponse();
        List seatbidList = new LinkedList<com.ad.model.sigmob.response.Seatbid>();
        for (com.ad.model.adx.response.SeatBid seatbid: adxResponse.getSeatbid()) {
            com.ad.model.sigmob.response.Seatbid resSeatbid = new com.ad.model.sigmob.response.Seatbid();
            List bidList = new LinkedList<com.ad.model.adx.response.Bid>();

            for (com.ad.model.adx.response.Bid bid: seatbid.getBid()) {
                com.ad.model.sigmob.response.Bid resBid = new com.ad.model.sigmob.response.Bid();
                resBid.setId(bid.getId());
                resBid.setImpid(bid.getImpid());
                resBid.setDealid(bid.getDealid());
                resBid.setPrice(bid.getPrice());
                resBid.setAdid(bid.getAdid());
                resBid.setAdomain(bid.getAdomain());
                resBid.setBundle(bid.getBundle());
                resBid.setCid(bid.getCid());
                resBid.setCrid(bid.getCrid());

                String resCat = "";
                for (String cat: bid.getCat()) {
                    resCat += "," + cat;
                }
                resBid.setCat(resCat);

                resBid.setAttr(3);
                resBid.setInteractiontype(Integer.valueOf(bid.getActionType()));
                resBid.setLandingpage(bid.getTarget());
                resBid.setDeeplinkurl(bid.getDeepLink());

                com.ad.model.sigmob.response.MaterialMeta materialMeta = new com.ad.model.sigmob.response.MaterialMeta();
                materialMeta.setTitle(bid.getApp().getName());
                resBid.setMaterial(materialMeta);

                List ttrackingList = new LinkedList<com.ad.model.sigmob.response.Ttracking>();
                ttrackingList.add(new com.ad.model.sigmob.response.Ttracking("imp", bid.getEvents().getEls()));
                ttrackingList.add(new com.ad.model.sigmob.response.Ttracking("click", bid.getEvents().getCls()));
                ttrackingList.add(new com.ad.model.sigmob.response.Ttracking("start", bid.getEvents().getSpls()));
                ttrackingList.add(new com.ad.model.sigmob.response.Ttracking("complete", bid.getEvents().getEpls()));
                ttrackingList.add(new com.ad.model.sigmob.response.Ttracking("skip", bid.getEvents().getSkls()));
                resBid.setTrackings(ttrackingList);

                bidList.add(resBid);
            }
            resSeatbid.setBid(bidList);
            resSeatbid.setSeat(seatbid.getSeat());

            seatbidList.add(resSeatbid);
        }

        sigmobResponse.setId(adxResponse.getId());
        sigmobResponse.setNbr(0);
        sigmobResponse.setSeatbid(seatbidList);

        return sigmobResponse;
    }
...
//                .addHeader("Accept-Encoding", "gzip")
        ...
        if (response.isSuccessful()) {
            ...
            if (result.getString("code").equals("10000")) {
                ADXResponse adxResponse = JSON.parseObject(body, ADXResponse.class);
                return ADXResponseToSigmobResponse(adxResponse);
            }

            throw new LogicException(result.getString("code"), result.getString("info"));
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        SigmobResponse sigmobResponse = new ADXUpStreamServiceImp().sigmobAd(sigmobRequest);
        System.out.println(sigmobResponse);
